...
class RecordService extends BaseFetcher {
  ...
  public getRecordByItemId = async (query: RecordItemIdQuerySchema): Promise<RecordResponseByItemIdSchema | null> => {
    const { itemId } = query;

    const response = await this.get<RecordResponseByItemIdSchema>(
      `/api/v1/records/item/${itemId}`,
      RECORD_RESPONSE_BY_ITEM_ID_SCHEMA
    );
