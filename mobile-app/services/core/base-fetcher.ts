...
import { getBetterAuthFetchHeaders } from "@/lib/auth-client";
import { getLogger, type Logger } from "@/lib/logger";

const resolveContentType = (headers: HeadersInit): string => {
  ...
  if (Array.isArray(headers)) {
    ...
    return found?.[1] ?? "application/json";
  }
  const record = headers as Record<string, string | undefined>;
  return record["Content-Type"] ?? record["content-type"] ?? "application/json";
};
  ...
  protected readonly fetcher = async <T>(url: string, options: FetchOptions = {}): Promise<T | null> => {
    try {
      ...
      const requestHeaders: HeadersInit = {
        ...
        ...getBetterAuthFetchHeaders(),
        ...headers
      };

      /**
       * Make request (Expo WinterCG fetch; session cookies on native via Better Auth storage).
       */
